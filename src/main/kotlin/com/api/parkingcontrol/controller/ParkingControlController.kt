...
//permite que seja acessado de qualquer ponto
...
class ParkingSpotController(val parkingSpotService: ParkingSpotService) {
    ...
    fun getOneParkingSpot(@PathVariable(value = "id") id: UUID?): ResponseEntity<Any> {
        ...
        return if (!parkingSpotModelOptional.isPresent) {
            ResponseEntity.status(HttpStatus.NOT_FOUND).body("Parking Spot not found.")
        } else ResponseEntity.status(HttpStatus.OK).body(parkingSpotModelOptional.get())
    }
    ...
    fun updateParkingSpot(
        ...
        @RequestBody parkingSpotDto: @Valid ParkingSpotDto?
    ): ResponseEntity<Any> {
        val parkingSpotModelOptional = parkingSpotService.findById(id!!)
        if (!parkingSpotModelOptional.isPresent) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Parking Spot not found.")
